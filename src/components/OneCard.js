...
const CardCard = ({sendCardData, eleindex, remove}) => {
    ...
    return (
        <div>
            ...
            <div className="card text-white bg-dark mb-3 d-inline-block">
                ...
                <small id="helpId" className="form-text text-muted" >Name of card</small>
                <div className="card-body">
                    <div className="form-group">
                        <label htmlFor="cardDescription"></label>
                        <textarea className="form-control bg-dark text-light" name="description" id="cardDescription" placeholder="describe card here" rows="3" onChange={(e)=>{setText(e.target.value)}}></textarea>
                    </div>
                    <button type="button" className="btn btn-danger" eleindex={eleindex} onClick={(e)=>{remove(e.target.eleindex)}}>Remove Card</button>
                </div>
            </div>
        </div>
    )
